    ...
    if is_train:
        ...
        with torch.inference_mode(False):
            with torch.enable_grad():
                ft_mean = functorch.vjp(_contract, mean)[0]
                ft_jacobian = functorch.vmap(
                    functorch.jacrev(_contract, argnums=0)
                )(mean_reshape)
...
def pos_enc(x, min_deg, max_deg, append_identity):
    ...
    xb = torch.reshape(
        (x[..., None, :] * scales[:, None]), x.shape[:-1] + (-1,)
    )
...
def searchsorted(a, v):
    ...
    idx_lo = (
        torch.where(v_ge_a, i[..., :, None], i[..., :1, None])
        .max(dim=-2)
        .values
    )
    idx_hi = (
...
def integrate_weights(w):
    ...
    shape = cw.shape[:-1] + (1,)
    cw0 = torch.cat(
        [torch.zeros(shape).type_as(cw), cw, torch.ones(shape).type_as(cw)],
        dim=-1,
    )
    return cw0
