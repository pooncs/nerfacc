...
__global__ void cdf_resampling_kernel(
    ...
    const int *resample_packed_info, scalar_t *resample_starts,
...
//     int idx = 0, j = 0;
//     scalar_t cdf_prev = 0.0f, cdf_next = (weights[idx] + padding_step) /
//     weights_sum; scalar_t cdf_u = 0.5f * cdf_step_size; while (cdf_u < 1.0f)
//     {
//         if (cdf_u < cdf_next)
//         {
//             // resample in this interval
//             scalar_t scaling = (ends[idx] - starts[idx]) / (cdf_next -
//             cdf_prev); scalar_t resample_mid = (cdf_u - cdf_prev) * scaling
//             + starts[idx]; scalar_t resample_half_size = cdf_step_size *
//             scaling * 0.5; resample_starts[j] = fmaxf(resample_mid -
//             resample_half_size, starts[idx]); resample_ends[j] =
//             fminf(resample_mid + resample_half_size, ends[idx]);
//             // going further to next resample
//             cdf_u += cdf_step_size;
//             j += 1;
//         }
...
std::vector<torch::Tensor> ray_resampling(torch::Tensor packed_info,
  ...
  torch::Tensor resample_num_steps =
      (num_steps > 0).to(num_steps.options()) * steps;
  torch::Tensor resample_cum_steps =
      resample_num_steps.cumsum(0, torch::kInt32);
  torch::Tensor resample_packed_info = torch::cat(
      {resample_cum_steps - resample_num_steps, resample_num_steps}, 1);

  int total_steps =
      resample_cum_steps[resample_cum_steps.size(0) - 1].item<int>();
  torch::Tensor resample_starts =
      torch::zeros({total_steps, 1}, starts.options());
  torch::Tensor resample_ends = torch::zeros({total_steps, 1}, ends.options());

  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      weights.scalar_type(), "ray_resampling", ([&] {
        cdf_resampling_kernel<scalar_t>
            <<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
                n_rays,
                // inputs
                packed_info.data_ptr<int>(), starts.data_ptr<scalar_t>(),
                ends.data_ptr<scalar_t>(), weights.data_ptr<scalar_t>(),
