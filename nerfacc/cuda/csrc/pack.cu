...
__global__ void unpack_info_kernel(
    // input
    const int n_rays, const int *packed_info,
    // output
    int64_t *ray_indices) {
...
__global__ void unpack_info_to_mask_kernel(
    // input
    const int n_rays, const int *packed_info, const int n_samples,
    // output
...
__global__ void unpack_data_kernel(
    ...
    const int data_dim, const scalar_t *data, const int n_sampler_per_ray,
...
torch::Tensor unpack_info(const torch::Tensor packed_info,
  ...
  torch::Tensor ray_indices =
      torch::empty({n_samples}, packed_info.options().dtype(torch::kLong));

  unpack_info_kernel<<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
      n_rays, packed_info.data_ptr<int>(), ray_indices.data_ptr<int64_t>());
  return ray_indices;
}
...
torch::Tensor unpack_info_to_mask(const torch::Tensor packed_info,
  ...
  unpack_info_to_mask_kernel<<<blocks, threads, 0,
                               at::cuda::getCurrentCUDAStream()>>>(
      n_rays, packed_info.data_ptr<int>(), n_samples, masks.data_ptr<bool>());
  return masks;
}
...
torch::Tensor unpack_data(torch::Tensor packed_info, torch::Tensor data,
  ...
  torch::Tensor unpacked_data =
      torch::zeros({n_rays, n_samples_per_ray, data_dim}, data.options());

  AT_DISPATCH_ALL_TYPES(
      data.scalar_type(), "unpack_data", ([&] {
        unpack_data_kernel<scalar_t>
            <<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
                n_rays,
                // inputs
                packed_info.data_ptr<int>(), data_dim,
                data.data_ptr<scalar_t>(), n_samples_per_ray,
                // outputs
