...
inline __device__ __host__ scalar_t
grid_occupied_at(const float3 xyz, const float3 roi_min, const float3 roi_max,
                 ContractionType type, int mip, const int grid_nlvl,
  ...
  int idx = grid_idx_at(xyz_unit, grid_res) +
            mip * grid_res.x * grid_res.y * grid_res.z;
...
inline __device__ __host__ float advance_to_next_voxel(
    ...
    const float3 inv_dir, int mip, const float3 roi_min, const float3 roi_max,
    const int3 grid_res, const float far) {
  // Regular stepping (may be slower but matches non-empty space)
  float t_target = t + distance_to_next_voxel(xyz, dir, inv_dir, mip, roi_min,
                                              roi_max, grid_res);
...
__global__ void ray_marching_kernel(
    ...
    const float *roi, const int grid_nlvl, const int3 grid_res,
    const bool *grid_binary,  // shape (reso_x, reso_y, reso_z)
    const ContractionType type,
    // sampling
    const float step_size, const float cone_angle, const int *packed_info,
    // first round outputs
    int *num_steps,
    // second round outputs
    int64_t *ray_indices, float *t_starts, float *t_ends) {
  ...
  const float grid_cell_scale = fmaxf(
      fmaxf((roi_max.x - roi_min.x) / grid_res.x * scalbnf(1.732f, grid_nlvl),
    ...
    if (grid_occupied_at(xyz, roi_min, roi_max, type, mip, grid_nlvl, grid_res,
      ...
      switch (type) {
        case ContractionType::AABB:
          // no contraction
          t_mid = advance_to_next_voxel(t_mid, dt_min, xyz, dir, inv_dir, mip,
...
std::vector<torch::Tensor> ray_marching(
  ...
  const int3 grid_res =
      make_int3(grid_binary.size(1), grid_binary.size(2), grid_binary.size(3));
  ...
  torch::Tensor num_steps =
      torch::empty({n_rays}, rays_o.options().dtype(torch::kInt32));

  // count number of samples per ray
  ray_marching_kernel<<<blocks, threads, 0,
                        at::cuda::getCurrentCUDAStream()>>>(
      // rays
      n_rays, rays_o.data_ptr<float>(), rays_d.data_ptr<float>(),
      t_min.data_ptr<float>(), t_max.data_ptr<float>(),
      // occupancy grid & contraction
      roi.data_ptr<float>(), grid_nlvl, grid_res, grid_binary.data_ptr<bool>(),
      type,
      // sampling
      step_size, cone_angle, nullptr, /* packed_info */
      // outputs
      num_steps.data_ptr<int>(), nullptr, /* ray_indices */
      nullptr,                            /* t_starts */
      nullptr /* t_ends */);

  torch::Tensor cum_steps = num_steps.cumsum(0, torch::kInt32);
  torch::Tensor packed_info =
      torch::stack({cum_steps - num_steps, num_steps}, 1);
  ...
  torch::Tensor ray_indices =
      torch::empty({total_steps}, cum_steps.options().dtype(torch::kLong));
  ...
  ray_marching_kernel<<<blocks, threads, 0,
      ...
      type,
      // sampling
      step_size, cone_angle, packed_info.data_ptr<int>(),
      // outputs
      nullptr, /* num_steps */
      ray_indices.data_ptr<int64_t>(), t_starts.data_ptr<float>(),
...
torch::Tensor grid_query(const torch::Tensor samples,
  ...
  const int3 grid_res =
      make_int3(grid_value.size(1), grid_value.size(2), grid_value.size(3));
  ...
  AT_DISPATCH_FLOATING_TYPES_AND(
      at::ScalarType::Bool, occs.scalar_type(), "grid_query", ([&] {
        query_occ_kernel<<<blocks, threads, 0,
                           at::cuda::getCurrentCUDAStream()>>>(
            n_samples, samples.data_ptr<float>(),
            // grid
            roi.data_ptr<float>(), grid_nlvl, grid_res,
            grid_value.data_ptr<scalar_t>(), type,
            // outputs
