...
__global__ void transmittance_from_sigma_forward_kernel(const uint32_t n_rays,
                                                        // inputs
...
__global__ void transmittance_from_sigma_backward_kernel(
    ...
    // inputs
    const int *packed_info, const float *starts, const float *ends,
    const float *transmittance, const float *transmittance_grad,
    // outputs
    float *sigmas_grad) {
  CUDA_GET_THREAD_ID(i, n_rays);
...
__global__ void transmittance_from_alpha_forward_kernel(const uint32_t n_rays,
                                                        // inputs
                                                        const int *packed_info,
                                                        const float *alphas,
                                                        // outputs
...
__global__ void transmittance_from_alpha_backward_kernel(
    const uint32_t n_rays,
    // inputs
    const int *packed_info, const float *alphas, const float *transmittance,
...
torch::Tensor transmittance_from_sigma_backward_naive(
  ...
  transmittance_from_sigma_backward_kernel<<<
      ...
      // inputs
      packed_info.data_ptr<int>(), starts.data_ptr<float>(),
      ends.data_ptr<float>(), transmittance.data_ptr<float>(),
...
torch::Tensor transmittance_from_alpha_backward_naive(
  ...
  transmittance_from_alpha_backward_kernel<<<
      ...
      // inputs
      packed_info.data_ptr<int>(), alphas.data_ptr<float>(),
      transmittance.data_ptr<float>(), transmittance_grad.data_ptr<float>(),
      // outputs
