...
inline void exclusive_sum_by_key(KeysInputIteratorT keys,
                                 ValuesInputIteratorT input,
                                 ValuesOutputIteratorT output,
                                 int64_t num_items) {
  TORCH_CHECK(
      num_items <= std::numeric_limits<int64_t>::max(),
...
template <typename KeysInputIteratorT, typename ValuesInputIteratorT,
          typename ValuesOutputIteratorT>
inline void exclusive_prod_by_key(KeysInputIteratorT keys,
                                  ValuesInputIteratorT input,
                                  ValuesOutputIteratorT output,
                                  int64_t num_items) {
  TORCH_CHECK(
  ...
  CUB_WRAPPER(cub::DeviceScan::ExclusiveScanByKey, keys, input, output,
              Product(), 1.0f, num_items, cub::Equality(),
...
#if CUB_SUPPORTS_SCAN_BY_KEY()
  exclusive_sum_by_key(ray_indices.data_ptr<int64_t>(),
                       sigmas_dt.data_ptr<float>(),
                       sigmas_dt_cumsum.data_ptr<float>(), n_samples);
#else
...
torch::Tensor transmittance_from_sigma_backward_cub(
  ...
  TORCH_CHECK(transmittance_grad.ndimension() == 2 &
              transmittance_grad.size(1) == 1);
  ...
  exclusive_sum_by_key(
      thrust::make_reverse_iterator(ray_indices.data_ptr<int64_t>() +
                                    n_samples),
      thrust::make_reverse_iterator(sigmas_dt_cumsum_grad.data_ptr<float>() +
                                    n_samples),
      thrust::make_reverse_iterator(sigmas_dt_grad.data_ptr<float>() +
                                    n_samples),
      n_samples);
#else
  std::runtime_error("CUB functions are only supported in CUDA >= 11.6.");
...
#if CUB_SUPPORTS_SCAN_BY_KEY()
  exclusive_prod_by_key(ray_indices.data_ptr<int64_t>(),
                        (1.0f - alphas).data_ptr<float>(),
                        transmittance.data_ptr<float>(), n_samples);
#else
  std::runtime_error("CUB functions are only supported in CUDA >= 11.6.");
#endif
  return transmittance;
...
torch::Tensor transmittance_from_alpha_backward_cub(
  ...
  std::runtime_error("CUB functions are only supported in CUDA >= 11.6.");
#endif
  torch::Tensor alphas_grad =
      sigmas_dt_grad / (1.0f - alphas).clamp_min(1e-10f);
  return alphas_grad;
}
