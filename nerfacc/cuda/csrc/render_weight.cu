...
__global__ void weight_from_sigma_forward_kernel(const uint32_t n_rays,
...
__global__ void weight_from_sigma_backward_kernel(
    const uint32_t n_rays, const int *packed_info, const float *starts,
...
__global__ void weight_from_alpha_forward_kernel(const uint32_t n_rays,
                                                 const int *packed_info,
                                                 const float *alphas,
                                                 // outputs
                                                 float *weights) {
...
__global__ void weight_from_alpha_backward_kernel(const uint32_t n_rays,
                                                  const int *packed_info,
                                                  const float *alphas,
                                                  const float *weights,
                                                  const float *grad_weights,
...
torch::Tensor weight_from_sigma_forward_naive(torch::Tensor packed_info,
  ...
  weight_from_sigma_forward_kernel<<<blocks, threads, 0,
                                     at::cuda::getCurrentCUDAStream()>>>(
      n_rays,
      // inputs
      packed_info.data_ptr<int>(), starts.data_ptr<float>(),
      ends.data_ptr<float>(), sigmas.data_ptr<float>(),
      // outputs
...
torch::Tensor weight_from_sigma_backward_naive(torch::Tensor weights,
  ...
  weight_from_sigma_backward_kernel<<<blocks, threads, 0,
                                      at::cuda::getCurrentCUDAStream()>>>(
      n_rays,
      // inputs
      packed_info.data_ptr<int>(), starts.data_ptr<float>(),
      ends.data_ptr<float>(), sigmas.data_ptr<float>(),
      weights.data_ptr<float>(), grad_weights.data_ptr<float>(),
      // outputs
...
torch::Tensor weight_from_alpha_forward_naive(torch::Tensor packed_info,
  ...
  weight_from_alpha_forward_kernel<<<blocks, threads, 0,
                                     at::cuda::getCurrentCUDAStream()>>>(
      n_rays,
      // inputs
      packed_info.data_ptr<int>(), alphas.data_ptr<float>(),
      // outputs
      weights.data_ptr<float>());
  return weights;
}
...
torch::Tensor weight_from_alpha_backward_naive(torch::Tensor weights,
  ...
  weight_from_alpha_backward_kernel<<<blocks, threads, 0,
                                      at::cuda::getCurrentCUDAStream()>>>(
      n_rays,
      // inputs
      packed_info.data_ptr<int>(), alphas.data_ptr<float>(),
