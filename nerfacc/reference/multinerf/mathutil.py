...
def log_lerp(t, v0, v1):
    ...
    lv0 = math.log(v0)
    lv1 = math.log(v1)
    return math.exp(clip(t, 0, 1) * (lv1 - lv0) + lv0)
...
def learning_rate_decay(
    ...
    if lr_delay_steps > 0:
        ...
        delay_rate = lr_delay_mult + (1 - lr_delay_mult) * math.sin(
            0.5 * math.pi * clip(step / lr_delay_steps, 0, 1)
        )
    else:
        delay_rate = 1.0
...
def interp(*args):
    """A gather-based vectorized replacement for _interp()."""
    args_flat = [x.reshape([-1, x.shape[-1]]) for x in args]
    ret = vmap(_interp)(*args_flat).reshape(args[0].shape)
    return ret
